...
async def run(bot, message):
    ...
    while True:
        try:
            chat = await meesage.reply_text("To Index a channel you may send me the channel invite link, so that I can join channel and index the files.\n\nIt should be something like <code>https://t.me/xxxxxx</code> or <code>https://t.me/joinchat/xxxxxx</code>")
            _text = await bot.listen(message.chat.id, filters=filters.text, timeout=90)
            if _text.text:
                channel = _text.text
                except TimeoutError:
